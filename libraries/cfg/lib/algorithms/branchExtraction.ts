...
export function extractBranchesFromProgram(cfp: ControlFlowProgram) {
  const paths: ControlFlowPath[] = [];
  for (const cff of cfp.functions) {
    paths.push(...extractBranchesFromFunction(cff));
  }

  return paths;
}

export function extractBranchesFromFunction(cff: ControlFlowFunction) {
  const paths: ControlFlowPath[] = [];
