 ...
 */
export class CrowdingDistance<T extends Encoding> implements Ranking<T> {
  /**
   * Compute the crowding distance for all individual int the front. This is a
   * variant to the classic crowding distance for many-objective problems, where
   * the extreme of the fronts receive a crowding distance equal to 2.0 while
   ...
   */
  rank(front: T[], objectiveFunctions: Set<ObjectiveFunction<T>>): T[][] {
...
/**
 * Factory plugin for CrowdingDistance
 *
 * @author Dimitri Stallenberg
 */
export class CrowdingDistanceFactory<T extends Encoding>
  implements RankingPlugin<T>
{
  name = "crowdingDistance";

  createRankingOperator(): CrowdingDistance<T> {
    return new CrowdingDistance<T>();
  }
}
