...
export function createAlgorithmFromConfig(
  ...
  const multiAlgorithms: {
    [key: string]: { new (...args: any[]): MultiGA<any> };
  } = {
    COMIX: COMIX,
  };
  ...
  if (algorithm in singleAlgoritms) {
    if (target instanceof Array) {
      throw new Error(
        `Cannot use multiple target for the single target algorithms.`
      );
    }
    ...
    if (target instanceof Target) {
      throw new Error(
        `Cannot use single target for the multi target algorithms.`
      );
    }
    ...
    if (!(subAlgorithm in singleAlgoritms)) {
      throw new Error(
        `${subAlgorithm} is not a valid sub-algorithm. (Multi-algorithms cannot use other multi-algorithms as sub-algorithms)`
      );
    }

    return new multiAlgorithms[algorithm](
      target,
      fitnessObject,
      sampler,
      singleAlgoritms[subAlgorithm]
    );
  } else {
