...
import { GeneticAlgorithm, Objective } from "..";
...
export function startOverTimeWriter(algo: GeneticAlgorithm) {
  ...
  overTimeWriter = setInterval(() => {
    ...
    if (!existsSync(file)) {
      writeToFile(
        file,
        `${gatherHeaderOutputValues(getProperty("output_properties"), algo)}\n`
      );
    }
...
function gatherHeaderOutputValues(
  outputValues: string[],
