...
export function loadConfig(args: any = {}): any {
    let config;
    let finalConfig;
    let log = args.logger ? args.logger.log : console.log;

    // Handle --config flag
    (args.config)
        ? config = path.join(args.workingDir, args.config)
        : config = path.join(args.workingDir, '.syntest.js');
