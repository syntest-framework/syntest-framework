 ...
 */
export class DynaMOSARankedAllPreset extends Preset {
  constructor() {
    super("DynaMOSA-ranked-all", "DynaMOSA preset");
  }
  ...
  modifyArgs<T>(arguments_: ArgumentsCamelCase<T>): void {
    ...
    if ("typePool" in arguments_) {
      (<{ typePool: boolean }>(<unknown>arguments_)).typePool = true;
    }

    if ("incorporateExecutionInformation" in arguments_) {
      (<{ incorporateExecutionInformation: boolean }>(
        (<unknown>arguments_)
      )).incorporateExecutionInformation = true;
    }
