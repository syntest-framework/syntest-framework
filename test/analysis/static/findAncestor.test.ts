import { BranchObjectiveFunction, CFG, Edge, NodeType, SearchSubject} from "../../../src";
...
describe("CFG ancestors search", function () {
    ...
    it("1 branch", () => {
        expect(cfgMini.findClosestAncestor("2", ["ROOT", "1", "3"])).to.eql([1, cfgMini.getNodeById("1")]);
    });

    it("CFG1 Target G, Path went to F", () => {
        // Path that was covered: A -> C -> D -> F -> A -> B
        // Try to find distance from G
        expect(CFG1.findClosestAncestor("G", ["ROOT", "A", "C", "D", "F", "A", "B"])).to.eql([1, CFG1.getNodeById("D")]);
    });

    it("CFG1 Target G, Path went to E", () => {
        // Path that was covered: A -> C -> E -> A -> B
        // Try to find distance from G
        expect(CFG1.findClosestAncestor("G", ["ROOT", "A", "C", "E", "A", "B"])).to.eql([2, CFG1.getNodeById("C")]);
    });

    it("CFG1 Target G, Path went to E and looped", () => {
        // Path that was covered: A -> C -> E -> A -> C -> D -> F -> A -> B
        // Try to find distance from G
        expect(CFG1.findClosestAncestor("G", ["ROOT", "A", "C", "E", "A", "C", "D", "F", "A", "B"])).to.eql([1, CFG1.getNodeById("D")]);
    });

    it("CFG1 Target E, Path went to B", () => {
        // Path that was covered: A -> B
        // Try to find distance from E
        expect(CFG1.findClosestAncestor("E", ["ROOT", "A", "B"])).to.eql([2, CFG1.getNodeById("A")]);
    });

    it("CFG1 Target F, Path went to B", () => {
        // Path that was covered: A -> B
        // Try to find distance from F
        expect(CFG1.findClosestAncestor("F", ["ROOT", "A", "B"])).to.eql([3, CFG1.getNodeById("A")]);
    });

    it("CFG2 Target E, Path went to S through H", () => {
        // Path that was covered: A -> C -> D -> F -> H -> K -> O -> P -> Q -> S
        // Try to find distance from E
        expect(CFG2.findClosestAncestor("E", ["ROOT", "A", "C", "D", "F", "H", "K", "N", "P", "Q", "S"])).to.eql([1, CFG2.getNodeById("C")]);
    });

    it("CFG2 Target I, Path went to S through H", () => {
        // Path that was covered: A -> C -> D -> F -> H -> K -> O -> P -> Q -> S
        // Try to find distance from I
        expect(CFG2.findClosestAncestor("I", ["ROOT", "A", "C", "D", "F", "H", "K", "N", "P", "Q", "S"])).to.eql([1, CFG2.getNodeById("F")]);
    });

    it("CFG2 Target M, Path went to S through H", () => {
        // Path that was covered: A -> C -> D -> F -> H -> K -> O -> P -> Q -> S
        // Try to find distance from M
        expect(CFG2.findClosestAncestor("M", ["ROOT", "A", "C", "D", "F", "H", "K", "N", "P", "Q", "S"])).to.eql([2, CFG2.getNodeById("F")]);
    });

    it("CFG2 Target N, Path went to S through H", () => {
        // Path that was covered: A -> C -> D -> F -> H -> K -> O -> P -> Q -> S
        // Try to find distance from N
        expect(CFG2.findClosestAncestor("N", ["ROOT", "A", "C", "D", "F", "H", "K", "N", "P", "Q", "S"])).to.eql([2, CFG2.getNodeById("F")]);
    });

    it("CFG2 Target R, Path went to S through H and Q", () => {
        // Path that was covered: A -> C -> D -> F -> H -> K -> O -> P -> Q -> S
        // Try to find distance from R
        expect(CFG2.findClosestAncestor("R", ["ROOT", "A", "C", "D", "F", "H", "K", "N", "P", "Q", "S"])).to.eql([1, CFG2.getNodeById("P")]);
    });

    it("CFG2 Target R, Path went to E and B", () => {
        // Path that was covered: A -> C -> E -> A -> B
        // Try to find distance from R
        expect(CFG2.findClosestAncestor("R", ["ROOT", "A", "C", "E", "A", "B"])).to.eql([3, CFG2.getNodeById("C")]);
    });

    it("CFG2 Target S, Path went to E and B", () => {
        // Path that was covered: A -> C -> E -> A -> B
        // Try to find distance from S
        expect(CFG2.findClosestAncestor("S", ["ROOT", "A", "C", "E", "A", "B"])).to.eql([3, CFG2.getNodeById("C")]);
    });

    it("CFG2 Target S, Path went to B immidiately", () => {
        // Path that was covered: A -> B
        // Try to find distance from S
        expect(CFG2.findClosestAncestor("S", ["ROOT", "A", "B"])).to.eql([4, CFG2.getNodeById("A")]);
    });

    it("CFG3 Target S, Path went to B immidiately", () => {
        // Path that was covered: A -> B
        // Try to find distance from S
        expect(CFG3.findClosestAncestor("S", ["ROOT", "A", "B"])).to.eql([2, CFG3.getNodeById("A")]);
    });

    it("CFG3 Target R, Path went through E", () => {
        // Path that was covered: A -> C -> E -> S
        // Try to find distance from S
        expect(CFG3.findClosestAncestor("R", ["ROOT", "A", "C", "E", "S"])).to.eql([3, CFG3.getNodeById("C")]);
    });
});